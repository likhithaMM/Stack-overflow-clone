...
const userByID = async (req, res, next, id) => {
  try {
    const user = await User.findById(id)
      .populate('following', '_id name') // Populate the 'following' field with selected fields
      .populate('followers', '_id name'); // Populate the 'followers' field with selected fields
...
const defaultPhoto = (req, res) => {
  ...
  const findPeople = async (req, res) => {
    ...
    try {
      let users = await User.findById({ _id: { $nin : following } }).select('name')
      //console.log("Found people:", users);
      res.json(users)
      //console.log("Controllers users",users)
      //console.log(users)
    }catch(err){
      return res.status(400).json({
        error: errorHandler.getErrorMessage(err)
